...
start_new_run <- readline(prompt = "Do you want to start a new run? (y/n): ")
start_new_run <- tolower(start_new_run) == "y"

# Set up checkpoint directory and file
checkpoint_dir <- "checkpoints"
dir_create(checkpoint_dir)
checkpoint_file <- path(checkpoint_dir, "last_checkpoint.rds")

# Configuration
batch_size <- 1  # Process one case at a time
cache_dir <- if (start_new_run) {
  # Create new cache directory for new run
  new_dir <- glue("cache/cache_{format(Sys.time(), '%Y%m%d_%H%M%S')}")
  dir_create(new_dir)
  new_dir
} else {
  # Use existing cache directory from checkpoint
  if (file_exists(checkpoint_file)) {
    checkpoint <- readRDS(checkpoint_file)
    checkpoint$cache_dir
  } else {
    stop("No checkpoint found. Please start a new run.")
  }
}
...
make_api_request <- function(valid_narratives, model = model_to_use, batch_id) {
  ...
  tryCatch({
    ...
    message(glue("\nMaking request for batch {batch_id}"))
    ...
    if (!is.data.frame(parsed)) {
      message("Converting parsed response to data frame")
    ...
    warning(glue("API request failed for batch {batch_id}: {e$message}"))
    return(NULL)
  })
}
...
process_batched <- function(df, col, narrative_type) {
  ...
  if (!start_new_run && file_exists(checkpoint_file)) {
    checkpoint <- readRDS(checkpoint_file)
    start_chunk <- checkpoint$last_chunk + 1
    message(glue("\nResuming from chunk {start_chunk} of {length(chunks)}"))
  } else {
    start_chunk <- 1
  }
  
  for (i in start_chunk:length(chunks)) {
    start_time <- Sys.time()
