...
batch_system_prompt <- paste(
  ...
  "",
  "For each narrative provided, you MUST return a single JSON object with the following 7 fields:",
  "{",
  "  \"sequence\": <integer, the original sequence number of the narrative>,",
  ...
  "  \"key_facts_summary\": \"A 1-2 sentence objective summary of the key events and circumstances described. State only the facts present in the narrative without interpretation.\"",
  "}",
  "",
  "Respond with a single, valid JSON array containing one object for each narrative in the order they were presented.",
...
openai_chat_batch <- function(narratives, model = "gpt-4o-mini", batch_id) {
  ...
  tibble(
    ...
    key_facts_summary = rep("parsing_error", length(narratives)) # <-- ADD THIS
  )
})
