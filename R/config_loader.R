...
validate_config <- function(config) {
  ...
  if (has_system_file) {
    if (!file.exists(config$prompt$system_prompt_file)) {
      stop("system_prompt_file not found: ", config$prompt$system_prompt_file)
    }
    config$prompt$system_prompt <- paste(readLines(config$prompt$system_prompt_file), collapse = "\n")
  }
  
  if (has_user_file) {
    if (!file.exists(config$prompt$user_template_file)) {
      stop("user_template_file not found: ", config$prompt$user_template_file)
    }
    config$prompt$user_template <- paste(readLines(config$prompt$user_template_file), collapse = "\n")
  }
  ...
  if (!file.exists(config$data$file)) {
...
expand_env_vars <- function(text) {
  ...
  while (grepl("\\$\\{[^}]+\\}", text, perl = TRUE)) {
    ...
    var_name <- gsub("\\$\\{|\\}", "", match)
    replacement <- Sys.getenv(var_name, unset = match)
