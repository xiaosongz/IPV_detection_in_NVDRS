...
-   **Average Accuracy**: `r fmt_pct(avg_accuracy)` ✅
-   **Average Precision**: `r fmt_pct(avg_precision)`
-   **Average Recall**: `r fmt_pct(avg_recall)` ⚠️ Needs Improvement
...
-   **False Negatives**: `r fmt_num(confusion$fn)` (`r fmt_pct(confusion$fn/total_predictions)`) - **Missed IPV cases** ⚠️
...
::: success-box
**📊 Optimal Configuration**: Temperature range **`r temp_summary$temp_bucket[which.max(temp_summary$f1)]`** achieves the best F1 score of `r fmt_dec(max(temp_summary$f1))`.
:::
...
-   **Use successful prompt features**: `r if(mean(prompt_analysis$has_examples) > 0.5) "✓ Include examples" else "• Add examples"`
...
if(quality$failed > 0) {
  cat(paste0("⚠️ **", quality$failed, " Failed Experiments** - Review error logs for root cause\n\n"))
}

stalled <- sum(experiments$status == "stalled")
if(stalled > 0) {
  cat(paste0("⚠️ **", stalled, " Stalled Experiments** - May need manual intervention\n\n"))
}

running <- sum(experiments$status == "running")
if(running > 0) {
  cat(paste0("ℹ️ **", running, " Running Experiments** - Wait for completion before final analysis\n\n"))
}
...
if(n_completed > 3) {
  ...
  if(nrow(outliers) > 0) {
    cat(paste0("⚠️ **", nrow(outliers), " Accuracy Outliers Detected** (>2σ from mean)\n\n"))
...
**Consistency: `r toupper(consistency_level)`** (F1 σ = `r fmt_dec(f1_sd)`)
...
If recall can be improved to **80%** while maintaining current precision: - New F1 Score: **\~0.78** (current: `r fmt_dec(avg_f1)`) - Reduction in missed cases: **\~40%** - Overall system reliability: **Substantially improved**
:::
...
**Database Connection:** - Host: `r Sys.getenv("PG_HOST")` - Port: `r Sys.getenv("PG_PORT")` - Database: `r Sys.getenv("PG_DATABASE")` - Report Generated: `r format(Sys.time(), "%Y-%m-%d %H:%M:%S %Z")`

**Analysis Parameters:** - Total Experiments Analyzed: `r n_exp` - Completed Experiments: `r n_completed` - Total Narratives: `r fmt_num(n_narratives)` - Date Range: `r format(min(completed$created_at, na.rm=TRUE), "%Y-%m-%d")` to `r format(max(completed$created_at, na.rm=TRUE), "%Y-%m-%d")`
