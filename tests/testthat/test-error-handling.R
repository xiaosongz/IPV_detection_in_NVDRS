...
test_that("handles missing user template placeholder", {
  config <- mock_config()
  config$prompt$user_template <- "No placeholder here"
...
test_that("handles zero division in metrics", {
  ...
  # Log negative-only results
  log_narrative_result(con, experiment_id, list(
    incident_id = "NEG1",
    narrative_type = "LE",
    row_num = 1,
    narrative_text = "Test negative case 1",
    manual_flag_ind = 0,
    manual_flag = 0,
    detected = FALSE,
    confidence = 0.9,
  ...
  log_narrative_result(con, experiment_id, list(
    ...
    narrative_type = "LE",
    row_num = 2,
    narrative_text = "Test negative case 2",
    manual_flag_ind = 0,
    manual_flag = 0,
    detected = FALSE,
    confidence = 0.8,
  ...
  # Finalize should handle zero division gracefully
  expect_error(
...
# trimws_safe function removed - not used in main pipeline
